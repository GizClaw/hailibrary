...
- Picture books live at `works/<level>/<category>/<subcategory>/<slug>/`, limited to `aa`, `a` through `n`. One series article derives into one picture book at one level. Its required `book.yaml` source is `source: {series}`; optional legacy `volume` and `volumes` default to `1`. The book has a complete beginning, development, turn, and resolution.
