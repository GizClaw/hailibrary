...
Produce a complete source work under `works/<level>/<category>/<subcategory>/<slug>/`. Follow `AGENTS.md`: each locale's `article.md` is authoritative for the literary work, YAML is authoritative for the adapted reader structures, and generated catalog JSON must not be edited.
...
1. Apply the selected Writer's `prompt` for creative viewpoint and its `language_prompt` for the locale novel's narration, character voice, dialogue, and naturalness.
2. During `$adapt-article`, apply the selected `prompts/levels/<level>.yaml` `prompt`, complete exact-level record, and locale vocabulary/reference records as the authoritative difficulty contract.
...
The novel is bound by safety, research, shared events, and non-imitation rules, but not by level mechanics. During adaptation, level requirements override Writer preferences. Do not proceed if any selected configuration lacks its required prompt field.

## Set the adaptation difficulty contract

After the novels pass review and before adaptation, read [references/level-and-vocabulary-contract.md](references/level-and-vocabulary-contract.md) and write a short working brief for **each locale**. The brief must state:
...
Treat the YAML files as authoritative. The reference is an operational map, not a substitute for reading the selected level's complete record. Do not begin adaptation until its brief shows both the mechanical ceiling and the complexity floor. A longer text, one difficult word, or one difficult question does not raise an otherwise simpler adaptation to a higher level.
...
## Write and review the novel

For each locale, let the selected Writer independently draft `locales/<locale>/article.md` from the shared events, learning goal, and research. Never translate or sentence-match another locale. Use `# <title>`, a blank line, and prose paragraphs separated by blank lines; optional `## ` headings are allowed for long works. Use no front matter, HTML, tables, images, pages, IDs, speaker metadata, vocabulary markers, or YAML.
...
Apply `$adapt-article` separately to every reviewed locale novel. It creates the level-bound visible `story.yaml` `article.pages[].paragraphs[]` on the shared page plan, with natural quotation and attribution and inline markers only on words already present. It may condense, cut subplots, simplify, and re-sentence, but must not invent or change the source story. If faithful adaptation exposes a source problem, edit and re-review `article.md`, then re-adapt.

Set `schema_version: 2`, define `audio_script.cast`, and apply `$scriptize-article` only to the completed adapted `article.pages`. Every block needs a stable ID for its future audio clip and subtitle cue. Speaker metadata must not manufacture visible punctuation or attribution.

## Author the shared work

Create `book.yaml`, `research.yaml`, `artwork.yaml`, every `locales/<locale>/article.md` and `story.yaml`, shared artwork, and any required vocabulary entries.
...
- Preserve the novel's coherent narrative or nonfiction arc while meeting the adaptation level.
- Define every narrator and character in `book.yaml` with stable IDs and usable visual/voice identities.
- Give every locale exactly the same page IDs, meaning, and illustration IDs; speaker sets remain aligned with the shared characters.
- Paginate each adaptation at real scene, paragraph, or action transitions. Do not rewrite pages as isolated summaries.
