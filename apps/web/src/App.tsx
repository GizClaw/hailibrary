...
import { bookCardLocaleView, loadBook, loadCatalogPageIndex, loadHome, loadIndex, loadLabels, loadSearch, loadStory, loadStyles, loadVocabulary, loadVocabularyCatalogPage, loadVocabularyPageIndex, loadWriters, resolveContentLocale, searchCards, sortReadingLevels, storyArticlePages, storyAudioPages, storyCast, storyPageBlocks, type ArticlePage, type Book, type BookCard, type CatalogPageIndex, type CatalogTaxonomy, type HomePageData, type LabelCatalog, type SiteIndex, type Story, type StoryContentPart, type StoryPage, type StyleProfile, type StyleSummary, type TaxonomyGroup, type VocabularyCatalogCard, type VocabularyEntry, type VocabularyPageIndex, type WriterProfile } from "./catalog";
...
function LibraryBookCard({ card, interfaceLocale, learningLocale, labelCatalog, taxonomy }: { card: BookCard; interfaceLocale: string; learningLocale: string; labelCatalog?: LabelCatalog | null; taxonomy?: CatalogTaxonomy }) {
  ...
  return <a className="book-card-link" href={`#/read/${view.card.path}?lang=${encodeURIComponent(view.contentLocale)}`}><Card className="catalog-book-card"><span className="catalog-cover"><img src={view.card.cover} alt={`${view.card.title} cover`} /><Badge>{view.level}</Badge></span><div><p className="catalog-book-meta">{view.subcategory} · {view.card.pageCount} {view.pageUnit}</p><h2>{view.card.title}</h2><p>{view.card.summary}</p>{view.labels.length > 0 && <div className="catalog-labels">{view.labels.map((label) => <Badge key={label} variant="secondary">{label}</Badge>)}</div>}<div className="catalog-book-footer"><span>{view.writerLabel} · {view.card.writer.displayName}</span></div></div></Card></a>;
}
...
function Reader({ path, locale, interfaceLocale }: { path: string; locale: string; interfaceLocale: string }) {
  ...
  useEffect(() => {
    ...
    loadBook(workId).then(async (book) => {
      ...
      const nextStory = await loadStory(workId, selectedLocale);
