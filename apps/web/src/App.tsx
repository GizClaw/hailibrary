...
import { bookCardLocaleView, catalogSeriesOptions, filterAndSortCatalogBySeries, filterArticles, flattenSeriesBooks, isMultiVolume, loadBook, loadCatalogPageIndex, loadHome, loadIndex, loadLabels, loadSearch, loadSeries, loadSeriesArticle, loadSeriesIndex, loadStory, loadStyles, loadVocabulary, loadVocabularyCatalogPage, loadVocabularyPageIndex, loadWriters, localizedValue, resolveContentLocale, searchCards, selectHomeSeriesCards, sortReadingLevels, storyArticlePages, storyAudioPages, storyCast, storyPageBlocks, type ArticleFilters, type ArticlePage, type Book, type BookCard, type CatalogPageIndex, type CatalogTaxonomy, type HomePageData, type LabelCatalog, type SeriesArticle, type SeriesCard, type SeriesIndex, type SeriesManifest, type SiteIndex, type Story, type StoryContentPart, type StoryPage, type StyleProfile, type StyleSummary, type TaxonomyGroup, type VocabularyCatalogCard, type VocabularyEntry, type VocabularyPageIndex, type WriterProfile } from "./catalog";
...
type Route = { page: "library" | "catalog" | "series" | "series-detail" | "writers" | "styles" | "vocabulary" | "reader" | "print" | "progress" | "about"; path?: string; locale?: string };
...
export function App() {
  ...
  return <div className="site-shell">
    ...
    {route.page === "series-detail" && route.path && <SeriesDetail id={route.path} locale={route.locale ?? interfaceLocale} interfaceLocale={interfaceLocale} />}
...
function Header({ route, locales, interfaceLocale, learningLocale, setInterface, setLearning, swap }: { route: Route; locales: string[]; interfaceLocale: string; learningLocale: string; setInterface: (v: string) => void; setLearning: (v: string) => void; swap: () => void }) {
  ...
  return <header className="site-header">
    ...
    <nav><a className={route.page === "catalog" ? "active" : ""} href="#/catalog">{text.library}</a><a className={route.page === "vocabulary" ? "active" : ""} href="#/vocabulary">{interfaceLocale === "zh-CN" ? "词汇表" : "Vocabulary"}</a><a className={route.page === "series" || route.page === "series-detail" ? "active" : ""} href="#/articles">{interfaceLocale === "zh-CN" ? "文章" : "Articles"}</a><a className={route.page === "writers" ? "active" : ""} href="#/writers">{interfaceLocale === "zh-CN" ? "作家" : "Writers"}</a><a className={route.page === "styles" ? "active" : ""} href="#/styles">{interfaceLocale === "zh-CN" ? "风格" : "Styles"}</a><a className={route.page === "progress" ? "active" : ""} href="#/progress">{text.progress}</a><a className={route.page === "about" ? "active" : ""} href="#/about">{text.about}</a></nav>
...
function SeriesPage({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  const filters = { query, type, classification, writer, style, topic, theme, mood, level };
  const filtered = useMemo(() => filterArticles(cards, filters, learningLocale), [cards, classification, learningLocale, level, mood, query, style, theme, topic, type, writer]);
  useEffect(() => {
    ...
    history.replaceState(null, "", `#/articles${params.size ? `?${params}` : ""}`);
  }, [classification, level, mood, query, style, theme, topic, type, writer]);
  ...
  const reset = () => { setQuery(""); setType("all"); setClassification("all"); setWriter("all"); setStyle("all"); setTopic("all"); setTheme("all"); setMood("all"); setLevel("all") };
  return <main className="series-page"><section className="profiles-hero series-hero"><div><p className="eyebrow">{chinese ? "原文与分级改编" : "Articles and adaptations"}</p><h1>{chinese ? "阅读完整文章" : "Read the complete articles"}</h1><p>{chinese ? "先读完整文章，再沿着不同等级探索它改编出的全部绘本。" : "Read each complete article, then explore every graded picture-book adaptation."}</p></div><BookOpen size={74} aria-hidden="true" /></section><section className="catalog-browser"><div className="catalog-search-row"><label className="search-field"><Search size={21} aria-hidden="true" /><span className="sr-only">{chinese ? "搜索文章" : "Search articles"}</span><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={chinese ? "搜索标题、类型或作家" : "Search title, type, or Writer"} />{query && <button type="button" className="clear-search" onClick={() => setQuery("")} aria-label={chinese ? "清除搜索" : "Clear search"}><X size={17} /></button>}</label><p><strong>{filtered.length}</strong> {chinese ? "篇文章" : filtered.length === 1 ? "article" : "articles"}</p></div><div className="catalog-body"><aside className="catalog-filters"><div className="filter-title"><SlidersHorizontal size={18} /><strong>{chinese ? "筛选文章" : "Filter articles"}</strong></div><CatalogFilter label={chinese ? "文章类型" : "Article type"} value={type} onChange={setType} all={chinese ? "全部" : "All"} options={Object.entries(index?.articleTypes ?? {}).map(([id, item]) => ({ value: id, label: localizedValue(item.names, interfaceLocale, id) }))} /><CatalogFilter label={chinese ? "分类" : "Category / genre"} value={classification} onChange={setClassification} all={chinese ? "全部" : "All"} options={classifications.map(([value, label]) => ({ value, label }))} /><CatalogFilter label={chinese ? "作家" : "Writer"} value={writer} onChange={setWriter} all={chinese ? "全部" : "All"} options={articleWriters.map((item) => ({ value: item.id, label: item.displayName }))} /><CatalogFilter label={chinese ? "风格" : "Style"} value={style} onChange={setStyle} all={chinese ? "全部" : "All"} options={styles.filter((item) => cards.some((card) => card.style === item.id)).map((item) => ({ value: item.id, label: styleName(item, interfaceLocale) }))} /><CatalogFilter label={labelGroupName(labelCatalog, "topics", interfaceLocale)} value={topic} onChange={setTopic} all={chinese ? "全部" : "All"} options={labelOptions("topics")} /><CatalogFilter label={labelGroupName(labelCatalog, "themes", interfaceLocale)} value={theme} onChange={setTheme} all={chinese ? "全部" : "All"} options={labelOptions("themes")} /><CatalogFilter label={labelGroupName(labelCatalog, "moods", interfaceLocale)} value={mood} onChange={setMood} all={chinese ? "全部" : "All"} options={labelOptions("moods")} /><CatalogFilter label={chinese ? "绘本级别" : "Picture-book level"} value={level} onChange={setLevel} all={chinese ? "全部" : "All"} options={levels.map((item) => ({ value: item, label: item.toUpperCase() }))} /><Button variant="ghost" onClick={reset}>{chinese ? "重置全部筛选" : "Reset all filters"}</Button></aside><div className="catalog-results">{!index ? <LoadingState locale={interfaceLocale} /> : filtered.length ? <div className="series-grid">{filtered.map((card) => { const locale = resolveContentLocale(learningLocale, card.availableLocales); const articleWriter = card.writers[locale] ?? Object.values(card.writers)[0]; return <a className="book-card-link" href={`#/articles/${card.id}?lang=${encodeURIComponent(locale)}`} key={card.id}><Card className="series-card"><img src={card.cover} alt="" /><div><p className="eyebrow">{localizedValue(card.typeNames ?? {}, interfaceLocale, card.genre)} · {card.genre}</p><h2>{localizedValue(card.titles, locale, card.id)}</h2><p>{chinese ? `作家 · ${articleWriter.displayName}` : `Writer · ${articleWriter.displayName}`}</p><span>{card.bookCount === 1 ? (chinese ? "1 本绘本" : "1 picture book") : (chinese ? `${card.bookSetCount} 套绘本 · ${card.bookCount} 册` : `${card.bookSetCount} sets · ${card.bookCount} books`)}</span></div></Card></a> })}</div> : <div className="catalog-empty"><Search size={30} /><h2>{chinese ? "没有找到文章" : "No articles found"}</h2><p>{chinese ? "换一个关键词，或者清除部分筛选条件。" : "Try another search or clear some filters."}</p></div>}</div></div></section></main>;
}
...
function SeriesDetail({ id, locale, interfaceLocale }: { id: string; locale: string; interfaceLocale: string }) {
  ...
  useEffect(() => { let cancelled = false; loadSeries(id).then((next) => { if (cancelled) return; const selected = resolveContentLocale(locale, next.availableLocales); setSeries(next); setContentLocale(selected); loadSeriesArticle(id, selected).then((value) => { if (!cancelled) setArticle(value) }) }); return () => { cancelled = true } }, [id, locale]);
  const switchLanguage = (next: string) => { location.hash = `/articles/${id}?lang=${encodeURIComponent(next)}` };
  const chinese = interfaceLocale === "zh-CN";
  if (!series || !article) return <main className="series-detail"><LoadingState locale={interfaceLocale} /> </main>;
  const writer = series.locales[contentLocale].writer;
  return <main className="series-detail"><article className="series-article" lang={contentLocale}><header className="series-detail-header"><img src={series.cover} alt="" /><div><p className="eyebrow">{series.category} · {series.genre}</p><h1>{article.title}</h1><p>{chinese ? `完整文章 · ${article.chapters.filter((chapter) => chapter.title).length || 1} 章 · ${series.bookCount} 本绘本改编` : `Complete article · ${article.chapters.filter((chapter) => chapter.title).length || 1} chapters · ${series.bookCount} picture-book ${series.bookCount === 1 ? "adaptation" : "adaptations"}`}</p><span>{chinese ? "作家" : "Writer"} · {writer.displayName}</span>{series.availableLocales.length > 1 && <label className="series-language"><small>{chinese ? "阅读语言" : "Reading language"}</small><Select value={contentLocale} onValueChange={switchLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{series.availableLocales.map((item) => <SelectItem key={item} value={item}>{localeName(item)}</SelectItem>)}</SelectContent></Select></label>}</div></header><div className="series-prose">{article.chapters.map((chapter, chapterIndex) => <section key={`${chapter.title}-${chapterIndex}`}>{chapter.title && <h2>{chapter.title}</h2>}{chapter.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</section>)}</div></article><section className="series-books"><p className="eyebrow">{chinese ? "分级绘本" : "Graded picture books"}</p><h2>{chinese ? "这篇文章的绘本" : "Picture books from this article"}</h2><Button asChild variant="outline"><a href={`#/catalog?series=${encodeURIComponent(series.id)}`}>{chinese ? "在绘本目录中查看" : "View in the picture-book catalog"}<ArrowRight size={15} aria-hidden="true" /></a></Button>{series.bookGroups.map((group) => <section key={group.level}><h3>{chinese ? "级别" : "Level"} {group.level.toUpperCase()}</h3><div className="series-book-grid">{group.books.map((book) => <a href={`#/read/${book.id}?lang=${encodeURIComponent(resolveContentLocale(contentLocale, Object.keys(book.titles)))}`} key={book.id}><img src={book.cover} alt="" /><div>{isMultiVolume(book) && <span>{chinese ? `第 ${book.volume}/${book.volumes} 册` : `Volume ${book.volume}/${book.volumes}`}</span>}<strong>{localizedValue(book.titles, contentLocale, book.id)}</strong></div></a>)}</div></section>)}</section></main>;
}
...
function WriterProfileCard({ writer, interfaceLocale }: { writer: WriterProfile; interfaceLocale: string }) {
  ...
  return <Card className="writer-profile-card"><a className="profile-portrait" href={worksUrl} aria-label={`${interfaceLocale === "zh-CN" ? "查看绘本：" : "View picture books by "}${writer.displayName}`}><img src={writer.avatar} alt={writer.displayName} /><Badge>{writer.recommendedLevels.map((level) => level.toUpperCase()).join(" · ")}</Badge></a><div className="profile-copy"><p className="eyebrow">{interfaceLocale === "zh-CN" ? "作家" : "Writer"} · {localeName(writer.locale)}</p><h2><a href={worksUrl}>{writer.displayName}</a></h2><div className="profile-tags">{writer.traits.slice(0, 3).map((trait) => <Badge key={trait} variant="secondary">{trait}</Badge>)}</div><Button asChild variant="outline" className="profile-works-link"><a href={worksUrl}><BookOpen size={15} aria-hidden="true" />{interfaceLocale === "zh-CN" ? "查看绘本" : "View picture books"}<ArrowRight size={15} aria-hidden="true" /></a></Button></div></Card>;
}
