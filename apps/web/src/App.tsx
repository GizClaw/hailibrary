...
function Library({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  const text = ui(interfaceLocale);
  useEffect(() => { loadHome(learningLocale).then(setHomeData) }, [learningLocale]);
  ...
  return <main>
    ...
    <section className="library" id="library"><div className="section-heading library-heading"><div><p className="eyebrow">{text.browse}</p><h2>{text.fit}</h2><p className="section-intro">{interfaceLocale === "zh-CN" ? "从书名、主题或原创 Writer 开始，找到下一本刚刚好的读物。" : "Start with a title, topic, or original Writer and find your next just-right read."}</p></div><div className="catalog-summary"><strong>{homeData?.bookCount ?? 0}</strong><span>{interfaceLocale === "zh-CN" ? "本故事" : "stories"}</span><i /><strong>{homeData?.localeCount ?? 0}</strong><span>{interfaceLocale === "zh-CN" ? "种语言" : "languages"}</span></div></div>
      ...
      <div className="home-showcase"><HomeCardRotator key={`${learningLocale}-${cardSignature}`} cards={filtered} interfaceLocale={interfaceLocale} learningLocale={learningLocale} start={text.start} taxonomy={homeData?.taxonomy} />
...
function HomeCardRotator({ cards, interfaceLocale, learningLocale, start, taxonomy }: { cards: BookCard[]; interfaceLocale: string; learningLocale: string; start: string; taxonomy?: CatalogTaxonomy }) {
  ...
  return <section className="home-book-grid home-card-rotator" aria-label={interfaceLocale === "zh-CN" ? "推荐故事" : "Featured stories"} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setPaused(false) }}>{rotation.slots.map((slot, slotIndex) => <div className="home-card-slot" key={slotIndex}>{slot.previous !== null && <div className="home-card-layer" data-layer="previous" aria-hidden="true" key={`${slot.revision}-previous-${cards[slot.previous].id}`}><HomeBookCard card={cards[slot.previous]} interfaceLocale={interfaceLocale} learningLocale={learningLocale} start={start} taxonomy={taxonomy} /></div>}<div className="home-card-layer" data-layer="current" key={`${slot.revision}-current-${cards[slot.current].id}`}><HomeBookCard card={cards[slot.current]} interfaceLocale={interfaceLocale} learningLocale={learningLocale} start={start} taxonomy={taxonomy} /></div></div>)}</section>;
}

function HomeBookCard({ card, interfaceLocale, learningLocale, start, taxonomy }: { card: BookCard; interfaceLocale: string; learningLocale: string; start: string; taxonomy?: CatalogTaxonomy }) {
  const translatedTitle = card.titles[interfaceLocale];
  return <Card className="home-book-card"><a className="home-book-cover" href={`#/read/${card.path}?lang=${learningLocale}`}><img src={card.cover} alt={`${card.title} cover`} /><Badge>{taxonomyName(taxonomy?.levels, card.level, interfaceLocale)}</Badge></a><div><p className="meta">{taxonomyName(taxonomy?.subcategories, card.subcategory, interfaceLocale)} · {card.pageCount} {interfaceLocale === "zh-CN" ? "页" : "pages"}</p><h3><a href={`#/read/${card.path}?lang=${learningLocale}`}>{card.title}</a></h3>{translatedTitle && translatedTitle !== card.title && <h4>{translatedTitle}</h4>}<p>{card.summaries[interfaceLocale] ?? card.summary}</p><div className="home-book-footer"><span>Writer · {card.writer.displayName}</span><div className="book-card-actions"><Button asChild variant="outline" size="sm"><a href={`#/print/${card.path}?lang=${learningLocale}`}><Printer size={15} aria-hidden="true" />{interfaceLocale === "zh-CN" ? "打印" : "Print"}</a></Button><Button asChild size="sm"><a href={`#/read/${card.path}?lang=${learningLocale}`}>{start}<ArrowRight size={15} aria-hidden="true" /></a></Button></div></div></div></Card>;
}
...
function Catalog({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  useEffect(() => { loadSearch(learningLocale).then(setCards) }, [learningLocale]);
  const levels = useMemo(() => sortReadingLevels([...new Set(cards.map((card) => card.level))]), [cards]);
  ...
  return <main className="catalog-page">
    ...
    <section className="catalog-browser">
      ...
      <div className="catalog-body"><aside className="catalog-filters"><div className="filter-title"><SlidersHorizontal size={18} /><strong>{interfaceLocale === "zh-CN" ? "筛选目录" : "Filter catalog"}</strong></div><CatalogFilter label={text.level} value={level} onChange={setLevel} all={text.all} options={levels.map((item) => ({ value: item, label: taxonomyName(pageIndex?.taxonomy.levels, item, interfaceLocale) }))} /><CatalogFilter label={interfaceLocale === "zh-CN" ? "分类" : "Category"} value={category} onChange={setCategory} all={text.all} options={categories.map((item) => ({ value: item, label: taxonomyName(pageIndex?.taxonomy.categories, item, interfaceLocale) }))} /><CatalogFilter label="Writer" value={writer} onChange={setWriter} all={text.all} options={writers.map((item) => ({ value: item.id, label: item.displayName }))} /><CatalogFilter label={interfaceLocale === "zh-CN" ? "风格" : "Style"} value={style} onChange={setStyle} all={text.all} options={styles.map((item) => ({ value: item.id, label: styleName(item, interfaceLocale) }))} /><CatalogFilter label={labelGroupName(labelCatalog, "topics", interfaceLocale)} value={topic} onChange={setTopic} all={text.all} options={topicOptions} /><CatalogFilter label={labelGroupName(labelCatalog, "themes", interfaceLocale)} value={theme} onChange={setTheme} all={text.all} options={themeOptions} /><CatalogFilter label={labelGroupName(labelCatalog, "moods", interfaceLocale)} value={mood} onChange={setMood} all={text.all} options={moodOptions} /><Button variant="ghost" onClick={() => { setQuery(""); setLevel("all"); setCategory("all"); setWriter("all"); setStyle("all"); setTopic("all"); setTheme("all"); setMood("all") }}>{interfaceLocale === "zh-CN" ? "重置全部筛选" : "Reset all filters"}</Button></aside>
        <div className="catalog-results">{filtered.length ? <div className="catalog-card-grid">{filtered.map((card) => <CatalogBookCard key={`${card.id}-${learningLocale}`} card={card} interfaceLocale={interfaceLocale} learningLocale={learningLocale} start={text.start} labelCatalog={labelCatalog} taxonomy={pageIndex?.taxonomy} />)}</div> : <div className="catalog-empty"><Search size={30} /><h2>{interfaceLocale === "zh-CN" ? "没有找到这本书" : "No stories found"}</h2><p>{interfaceLocale === "zh-CN" ? "换一个关键词，或者清除部分筛选条件。" : "Try another search or clear some filters."}</p></div>}</div></div>
    </section>
  </main>;
}
...
function CatalogBookCard({ card, interfaceLocale, learningLocale, start, labelCatalog, taxonomy }: { card: BookCard; interfaceLocale: string; learningLocale: string; start: string; labelCatalog?: LabelCatalog | null; taxonomy?: CatalogTaxonomy }) {
  const translatedTitle = card.titles[interfaceLocale];
  ...
  return <Card className="catalog-book-card"><a className="catalog-cover" href={`#/read/${card.path}?lang=${learningLocale}`}><img src={card.cover} alt={`${card.title} cover`} /><Badge>{taxonomyName(taxonomy?.levels, card.level, interfaceLocale)}</Badge></a><div><p className="catalog-book-meta">{taxonomyName(taxonomy?.subcategories, card.subcategory, interfaceLocale)} · {card.pageCount} {interfaceLocale === "zh-CN" ? "页" : "pages"}</p><h2><a href={`#/read/${card.path}?lang=${learningLocale}`}>{card.title}</a></h2>{translatedTitle && translatedTitle !== card.title && <h3>{translatedTitle}</h3>}<p>{card.summaries[interfaceLocale] ?? card.summary}</p>{labels.length > 0 && <div className="catalog-labels">{labels.map((label) => <Badge key={label} variant="secondary">{label}</Badge>)}</div>}<div className="catalog-book-footer"><span>Writer · {card.writer.displayName}</span><div className="book-card-actions"><Button asChild variant="outline" size="sm"><a href={`#/print/${card.path}?lang=${learningLocale}`}><Printer size={14} aria-hidden="true" />{interfaceLocale === "zh-CN" ? "打印" : "Print"}</a></Button><Button asChild size="sm"><a href={`#/read/${card.path}?lang=${learningLocale}`}>{start}<ArrowRight size={15} aria-hidden="true" /></a></Button></div></div></div></Card>;
}
...
function VocabularyPage({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  useEffect(() => {
    ...
    if (!url) { setEntries([]); return }
    let cancelled = false;
    loadVocabularyCatalogPage(url).then((nextEntries) => { if (!cancelled) setEntries(nextEntries) });
  ...
  return <main className="vocabulary-page">
    ...
    <section className="vocabulary-browser">
      ...
      {pageCount > 1 && <nav className="vocabulary-pagination" aria-label={interfaceLocale === "zh-CN" ? "词汇分页" : "Vocabulary pages"}><Button variant="outline" disabled={page === 0} onClick={() => setPage((current) => current - 1)}><ArrowLeft size={16} />{interfaceLocale === "zh-CN" ? "上一页" : "Previous"}</Button><span>{page + 1} / {pageCount}</span><Button variant="outline" disabled={page >= pageCount - 1} onClick={() => setPage((current) => current + 1)}>{interfaceLocale === "zh-CN" ? "下一页" : "Next"}<ArrowRight size={16} /></Button></nav>}
    </section>
...
function WritersPage({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  const [query, setQuery] = useState("");
  useEffect(() => { loadWriters(learningLocale).then(setWriters) }, [learningLocale]);
...
type PrintablePage =
  | { kind: "cover" }
  | { kind: "story"; page: Story["pages"][number]; number: number }
...
function printableLine(line: Story["pages"][number]["lines"][number]) {
...
function PrintBook({ path, locale }: { path: string; locale: string }) {
  ...
  if (!book || !story) return <main className="print-loading">{locale === "zh-CN" ? "正在准备打印版……" : "Preparing printable edition…"}</main>;
  ...
  const insidePages: PrintablePage[] = [
    ...story.pages.map((page, index): PrintablePage => ({ kind: "story", page, number: index + 1 })),
    ...vocabularyPages,
    ...questionPages,
  ];
...
function PrintArticle({ book, story, locale, vocabulary }: { book: Book; story: Story; locale: string; vocabulary: Array<{ id: string; entry: VocabularyEntry }> }) {
  ...
  return <main className="print-view print-view--article" lang={locale}>
    <header className="print-toolbar"><a href="#/catalog"><ArrowLeft size={17} aria-hidden="true" />{chinese ? "返回图书馆" : "Back to library"}</a><div><strong>{story.title}</strong><span>{chinese ? `长文打印 · ${story.pages.length} 节` : `Article print · ${story.pages.length} sections`}</span></div>{book.availableLocales.length > 1 && <label><small>{chinese ? "打印语言" : "Print language"}</small><Select value={locale} onValueChange={switchLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{book.availableLocales.map((bookLocale) => <SelectItem value={bookLocale} key={bookLocale}>{localeName(bookLocale)}</SelectItem>)}</SelectContent></Select></label>}<p>{chinese ? "A4 竖向 · 文章排版 · 自动分页" : "A4 portrait · article layout · automatic pagination"}</p><Button onClick={() => window.print()}><Printer size={17} aria-hidden="true" />{chinese ? "打印" : "Print"}</Button></header>
    <section className="print-article-preview"><article className="print-article-paper">
      ...
      <div className="print-article-body">{story.pages.map((page, index) => <section className="print-article-section" key={page.id}><p className="print-article-folio">{chinese ? `第 ${index + 1} 节` : `Section ${index + 1}`}</p>{page.lines.map((line, lineIndex) => <div className="print-article-line" key={`${page.id}-${lineIndex}`}><small>{story.cast[line.speaker]?.display_name}</small><p>{printableLine(line)}</p></div>)}</section>)}</div>
...
function PrintableBookPage({ page, book, story, locale, position }: { page: PrintablePage; book: Book; story: Story; locale: string; position: "left" | "right" }) {
  ...
  if (page.kind === "story") return <section className="print-book-page print-story-page" data-position={position}><img src={book.artwork.pages[page.page.illustration]} alt="" /><div className="print-story-copy">{page.page.lines.map((line, index) => <div key={`${page.page.id}-${index}`}><small>{story.cast[line.speaker]?.display_name}</small><p>{printableLine(line)}</p></div>)}</div><footer><span>{page.number}</span><small>{chinese ? "内容由 AI 生成" : "Content generated by AI"}</small></footer></section>;
...
function Reader({ path, locale, interfaceLocale }: { path: string; locale: string; interfaceLocale: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadBook(workId), loadStory(workId, locale)]).then(async ([book, nextStory]) => {
      const ids = [...new Set(nextStory.pages.flatMap((item) => item.lines.flatMap((line) => line.content?.flatMap((part) => part.vocabulary?.id ?? []) ?? [])))];
      ...
      setPage(Math.min(targetBook?.page ?? 0, Math.max(0, nextStory.pages.length - 1)));
      setActiveWord(null);
      setStory(nextStory);
    });
    return () => { cancelled = true };
  }, [path, locale]);
  useEffect(() => { if (story?.language === locale) savePage(workId, locale, page, story.pages.length) }, [workId, locale, page, story]);
  ...
  useEffect(() => {
    if (!articleLayout || !story) return;
    const sections = Array.from(document.querySelectorAll<HTMLElement>("[data-article-page]"));
    const observer = new IntersectionObserver((entries) => {
      ...
      if (visible) setPage(Number((visible.target as HTMLElement).dataset.articlePage ?? 0));
    }, { rootMargin: "-18% 0px -68%", threshold: 0 });
    sections.forEach((section) => observer.observe(section));
    return () => observer.disconnect();
  }, [articleLayout, story]);
  if (!story) return <main className="loading">正在打开故事…</main>;
  const current = story.pages[page];
  const questions = story.questions ?? [];
  const question = questions[questionIndex];
  const evidencePage = question ? story.pages.find((item) => item.id === question.page_refs[0]) ?? current : current;
...
function ArticleStory({ story, locale, level, cover, activeWord, activeEntry, activeLocale, onOpenWord, aiDisclosure }: { story: Story; locale: string; level: string; cover: string; activeWord: string | null; activeEntry?: VocabularyEntry; activeLocale?: VocabularyEntry["locales"][string]; onOpenWord: (id: string | null) => void; aiDisclosure: string }) {
  const chinese = locale === "zh-CN";
  return <article className="article-reading-page"><header className="article-reading-header"><img src={cover} alt="" /><div><p className="eyebrow">{chinese ? `长文阅读 · ${story.pages.length} 节` : `Long-form reading · ${story.pages.length} sections`}</p><h1>{story.title}</h1><p>{story.summary}</p><span>{chinese ? "作者" : "Writer"} · {story.writer}　·　{chinese ? "级别" : "Level"} {level.toUpperCase()}</span></div></header><div className="article-reading-copy">{activeWord && activeEntry && activeLocale && <aside className="vocabulary-card article-vocabulary-card" aria-live="polite"><img src={`./vocabulary/${level}/${activeWord}/card.webp`} alt="" /><div><div className="vocabulary-heading"><span>{activeLocale.part_of_speech}</span><button type="button" onClick={() => onOpenWord(null)} aria-label={chinese ? "关闭词卡" : "Close word card"}><X size={16} aria-hidden="true" /></button></div><h2>{activeLocale.term}</h2>{activeLocale.pronunciation && <p className="pronunciation">{activeLocale.pronunciation}</p>}<p>{activeLocale.definition}</p></div></aside>}{story.pages.map((page, pageIndex) => <section className="article-section" data-article-page={pageIndex} key={page.id}><p className="article-section-label">{chinese ? `第 ${pageIndex + 1} 节` : `Section ${pageIndex + 1}`}</p>{page.lines.map((line, lineIndex) => <div className="article-line" key={`${page.id}-${lineIndex}`}><small>{story.cast[line.speaker]?.display_name}</small><p><StoryText text={line.text} content={line.content} activeWord={activeWord} onOpen={onOpenWord} /></p></div>)}</section>)}<p className="reader-ai-disclosure"><Sparkles size={12} aria-hidden="true" />{aiDisclosure}</p></div></article>;
}
