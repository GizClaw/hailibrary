...
import { bookCardLocaleView, catalogSeriesOptions, filterAndSortCatalogBySeries, flattenSeriesBooks, loadBook, loadCatalogPageIndex, loadHome, loadIndex, loadLabels, loadSearch, loadSeries, loadSeriesArticle, loadSeriesIndex, loadStory, loadStyles, loadVocabulary, loadVocabularyCatalogPage, loadVocabularyPageIndex, loadWriters, localizedValue, resolveContentLocale, searchCards, searchSeries, selectHomeSeriesCards, sortReadingLevels, storyArticlePages, storyAudioPages, storyCast, storyPageBlocks, type ArticlePage, type Book, type BookCard, type CatalogPageIndex, type CatalogTaxonomy, type HomePageData, type LabelCatalog, type SeriesArticle, type SeriesCard, type SeriesManifest, type SiteIndex, type Story, type StoryContentPart, type StoryPage, type StyleProfile, type StyleSummary, type TaxonomyGroup, type VocabularyCatalogCard, type VocabularyEntry, type VocabularyPageIndex, type WriterProfile } from "./catalog";
...
export function App() {
  ...
  return <div className="site-shell">
    ...
    {route.page === "series" && <SeriesPage interfaceLocale={interfaceLocale} />}
...
function Header({ route, locales, interfaceLocale, learningLocale, setInterface, setLearning, swap }: { route: Route; locales: string[]; interfaceLocale: string; learningLocale: string; setInterface: (v: string) => void; setLearning: (v: string) => void; swap: () => void }) {
  ...
  return <header className="site-header">
    ...
    <nav><a className={route.page === "catalog" ? "active" : ""} href="#/catalog">{text.library}</a><a className={route.page === "vocabulary" ? "active" : ""} href="#/vocabulary">{interfaceLocale === "zh-CN" ? "词汇表" : "Vocabulary"}</a><a className={route.page === "series" || route.page === "series-detail" ? "active" : ""} href="#/series">{interfaceLocale === "zh-CN" ? "作品" : "Works"}</a><a className={route.page === "writers" ? "active" : ""} href="#/writers">{interfaceLocale === "zh-CN" ? "作家" : "Writers"}</a><a className={route.page === "styles" ? "active" : ""} href="#/styles">{interfaceLocale === "zh-CN" ? "风格" : "Styles"}</a><a className={route.page === "progress" ? "active" : ""} href="#/progress">{text.progress}</a><a className={route.page === "about" ? "active" : ""} href="#/about">{text.about}</a></nav>
...
function SeriesPage({ interfaceLocale }: { interfaceLocale: string }) {
  const [cards, setCards] = useState<SeriesCard[]>([]);
  const [query, setQuery] = useState("");
  const [loading, setLoading] = useState(true);
  useEffect(() => { let cancelled = false; loadSeriesIndex().then((index) => { if (!cancelled) { setCards(index.series); setLoading(false) } }); return () => { cancelled = true } }, []);
  const filtered = useMemo(() => searchSeries(cards, query), [cards, query]);
  const chinese = interfaceLocale === "zh-CN";
  return <main className="series-page"><section className="profiles-hero series-hero"><div><p className="eyebrow">{chinese ? "原作与分级改编" : "Original works and adaptations"}</p><h1>{chinese ? "阅读完整作品" : "Read the complete works"}</h1><p>{chinese ? "先读完整的系列文章，再沿着不同等级探索它改编出的全部绘本。" : "Read each complete series article, then explore every graded picture-book adaptation."}</p></div><BookOpen size={74} aria-hidden="true" /></section><section className="profiles-browser"><label className="search-field"><Search size={21} aria-hidden="true" /><span className="sr-only">{chinese ? "搜索作品" : "Search works"}</span><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={chinese ? "搜索标题、类型或作家" : "Search title, type, or Writer"} />{query && <button type="button" className="clear-search" onClick={() => setQuery("")} aria-label={chinese ? "清除搜索" : "Clear search"}><X size={17} /></button>}</label><p className="profile-count"><strong>{filtered.length}</strong> {chinese ? "篇作品" : filtered.length === 1 ? "work" : "works"}</p>{loading ? <LoadingState locale={interfaceLocale} /> : <div className="series-grid">{filtered.map((card) => { const locale = resolveContentLocale(interfaceLocale, card.availableLocales); const title = localizedValue(card.titles, locale, card.id); const writer = card.writers[locale] ?? Object.values(card.writers)[0]; return <a className="book-card-link" href={`#/series/${card.id}?lang=${encodeURIComponent(locale)}`} key={card.id}><Card className="series-card"><img src={card.cover} alt="" /><div><p className="eyebrow">{card.category} · {card.genre}</p><h2>{title}</h2><p>{chinese ? `作家 · ${writer.displayName}` : `Writer · ${writer.displayName}`}</p><span>{chinese ? `${card.bookSetCount} 套绘本 · ${card.bookCount} 册` : `${card.bookSetCount} sets · ${card.bookCount} books`}</span></div></Card></a> })}</div>}</section></main>;
}
...
function SeriesDetail({ id, locale, interfaceLocale }: { id: string; locale: string; interfaceLocale: string }) {
  ...
  const switchLanguage = (next: string) => { location.hash = `/series/${id}?lang=${encodeURIComponent(next)}` };
  const chinese = interfaceLocale === "zh-CN";
  ...
  return <main className="series-detail"><article className="series-article" lang={contentLocale}><header className="series-detail-header"><img src={series.cover} alt="" /><div><p className="eyebrow">{series.category} · {series.genre}</p><h1>{article.title}</h1><p>{chinese ? `完整作品 · ${article.chapters.filter((chapter) => chapter.title).length || 1} 章 · ${series.bookCount} 册绘本改编` : `Complete work · ${article.chapters.filter((chapter) => chapter.title).length || 1} chapters · ${series.bookCount} picture-book adaptations`}</p><span>{chinese ? "作家" : "Writer"} · {writer.displayName}</span>{series.availableLocales.length > 1 && <label className="series-language"><small>{chinese ? "阅读语言" : "Reading language"}</small><Select value={contentLocale} onValueChange={switchLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{series.availableLocales.map((item) => <SelectItem key={item} value={item}>{localeName(item)}</SelectItem>)}</SelectContent></Select></label>}</div></header><div className="series-prose">{article.chapters.map((chapter, chapterIndex) => <section key={`${chapter.title}-${chapterIndex}`}>{chapter.title && <h2>{chapter.title}</h2>}{chapter.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</section>)}</div></article><section className="series-books"><p className="eyebrow">{chinese ? "分级绘本" : "Graded picture books"}</p><h2>{chinese ? "这篇作品的绘本" : "Picture books from this work"}</h2><Button asChild variant="outline"><a href={`#/catalog?series=${encodeURIComponent(series.id)}`}>{chinese ? "在绘本目录中查看" : "View in the picture-book catalog"}<ArrowRight size={15} aria-hidden="true" /></a></Button>{series.bookGroups.map((group) => <section key={group.level}><h3>{chinese ? "级别" : "Level"} {group.level.toUpperCase()}</h3><div className="series-book-grid">{group.books.map((book) => <a href={`#/read/${book.id}?lang=${encodeURIComponent(resolveContentLocale(contentLocale, Object.keys(book.titles)))}`} key={book.id}><img src={book.cover} alt="" /><div><span>{chinese ? `第 ${book.volume}/${book.volumes} 册` : `Volume ${book.volume}/${book.volumes}`}</span><strong>{localizedValue(book.titles, contentLocale, book.id)}</strong></div></a>)}</div></section>)}</section></main>;
}
...
function Reader({ path, locale, interfaceLocale }: { path: string; locale: string; interfaceLocale: string }) {
  ...
  useEffect(() => {
    ...
    loadBook(workId).then(async (book) => {
      ...
      setSourceMeta({ series: sourceSeries, volume: book.source.volume, volumes: book.source.volumes, previous: siblings[siblingIndex - 1]?.id, next: siblings[siblingIndex + 1]?.id });
