...
import { loadBook, loadCatalogPageIndex, loadHome, loadIndex, loadLabels, loadSearch, loadStory, loadStyles, loadVocabulary, loadVocabularyCatalogPage, loadVocabularyPageIndex, loadWriters, searchCards, type Book, type BookCard, type CatalogPageIndex, type CatalogTaxonomy, type HomePageData, type LabelCatalog, type SiteIndex, type Story, type StoryContentPart, type StyleProfile, type StyleSummary, type TaxonomyGroup, type VocabularyCatalogCard, type VocabularyEntry, type VocabularyPageIndex, type WriterProfile } from "./catalog";
...
function Library({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  const levels = useMemo(() => [...new Set(cards.map((card) => card.level))].sort(), [cards]);
...
function VocabularyPage({ interfaceLocale, learningLocale }: { interfaceLocale: string; learningLocale: string }) {
  ...
  const availableLevels = useMemo(() => Object.keys(index?.locales[learningLocale] ?? {}).sort(), [index, learningLocale]);
  useEffect(() => {
